...
from backend.kafka.data_pipeline.ingestion.stream_ingestion import stream_ingestion_manager
...
async def fetch_stream_status() -> Dict[str, Any]:
    ...
    status = stream_ingestion_manager.get_status()
    return {
        "status": "streaming" if status["is_running"] else "idle",
...
async def fetch_stream_metrics() -> Dict[str, Any]:
    ...
    metrics = stream_ingestion_manager.get_metrics()
    return {
        "metrics": {
            ...
            "latency_ms": metrics["recent_latency_ms"],
            "mode": metrics["active_mode"]
        }
    }


async def fetch_recent_stream_data():
    """Retrieve recent records ingested through the streaming ledger."""
    transactions = await get_all_transactions(page=1, page_size=20)
    return {
